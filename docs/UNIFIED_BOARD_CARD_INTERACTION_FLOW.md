...
Board Interaction Regression Pass locks the current presentation behavior with focused tests: opening or closing discard / hand-picker overlays does not mutate `GameState`, does not submit commands unless the user explicitly confirms an existing choice, and hides the global FloatingActionPair to avoid duplicate controls. Toast presentation is independent from the full event log. Switching `viewingPlayerId` through player avatars remains read-only presentation state; it never changes `activePlayerId`, never sends `PlayerCommand`, and cannot stage play / payment / pending targets on an opponent board.

## Ability Target Interaction Polish

Simple board-target rewards use a shorter presentation path. When an active pending choice has exactly one selectable `placeEgg`, `placeYoung`, `hatchEgg`, or matching-fish egg entry, `GameBoardViewModel` automatically stages that entry and immediately highlights legal board targets. This is a presentation shortcut only: tapping a target still submits the existing pending-choice command, final validation remains in the engine, and compound abilities still require the player to choose an effect first.

`BoardInteractionPromptViewState` identifies the current choose-reward, choose-target, choose-source, choose-hand-card, choose-discard-card, or payment phase. These neutral instructions are rendered separately from `errorMessage`; red error styling is reserved for an actually invalid source or target. Optional skip and finish actions use explicit `跳过` / `结束` labels in `FloatingActionPairView` instead of an ambiguous forward arrow.

The board canvas uses one ordered hit-test stack: mapped highlight, non-interactive card face, transparent slot target, then resource-token payment targets when applicable. This prevents the visual card layer from swallowing target taps while preserving direct token payment selection. Authoritative refresh reconciles and removes staged reward state when its pending choice disappears or changes; it never implements committed undo. Board resource token insertion and selection use stable identity plus lightweight token transitions.

The board canvas now uses the bundled physical player-mat background and `player_mat_layout.json`. Every played fish uses a card-sized rect aligned to the printed slot instead of a second inset placeholder. Normal empty slots remain transparent, and the three printed forage fish come from the mat artwork; a real played fish still renders through `FishCardFaceView` above that background. S&R adds the physical coral strip above Twilight while reusing the exact same 18-slot mapping and command flow.
